...
chem_sum <- chem_groups %>%
  ...
  complete(date, Louis_class) %>% # make sure each day has all species
...
ggsave(filename="fig2.png", fig2, width=16, height=14, dpi=300)
```
...
ggsave(filename="fig5.png", fig5, width=16, height=10, dpi=300)
```
...
ggsave(filename="fig7.png", fig7, width=16, height=18, dpi=300)
...
ggsave(filename="fig8_cyano.png", fig8, width=16, height=18, dpi=300)
...
ggsave(filename="fig10_cryp.png", fig10, width=16, height=18, dpi=300)
```
```{r}
...
```
```{r}
#Make timeseries comparison for raph 
p1 <- chem_compare %>% 
  pivot_longer(c(TChla_mean:micro_rel), 
               names_to = "type", values_to = "bio_abun") %>% 
  filter(phyto_group == "raph" & (type == "chem_rel" | type == "micro_rel")) %>% 
  ggplot(aes(date, bio_abun, color = type)) +
  geom_line(size = 1) +
  scale_x_date(limits = startEnd,
               breaks = scales::date_breaks("1 month"),
  ...
  theme(legend.position = c(0.92, 0.9),
        legend.background =  element_blank(),
        text = element_text(size = 25)) +
  labs(y = "Relative Abund. (%)", title = "Raphidophyta")
...
p3 <- chem_compare %>%
  ...
  theme(axis.title.x = element_blank(),
        axis.text.x = element_blank(),
        text = element_text(size = 25)) +
  labs(y = bquote("Raph. (mg" ~ m^-3*")"))

p4 <- hplc_qu39_tidy %>% 
  filter(date > "2018-01-01" & date < "2018-12-31" & (pigments == "viola" |
                                                      pigments == "chl_c1_c2")) %>%
  group_by(date, pigments) %>% 
  summarise(concentration_mean = mean(concentration)) %>% 
  ggplot(aes(date, concentration_mean, color = fct_rev(pigments))) +
...
p6 <- chem_groups %>% 
  filter(Louis_class == "Raphidiophyta" &
           date > "2018-01-01" &
           date < "2018-12-31") %>%
  complete(date, Louis_class, scientificName_accepted) %>% 
  mutate(count = replace_na(count, 0)) %>% 
...
fig12 <- p1/p2/p3/p4/p5/p6 + plot_layout(ncol = 1, 
                                             heights = c(1, 0.3, 1, 1, 1, 1))

ggsave(filename="fig12_Raph.png", fig12, width=16, height=20, dpi=300)
```
```{r}
...
ggsave(filename="fig13_Dict.png", fig13, width=16, height=20, dpi=300)
```
```{r}
